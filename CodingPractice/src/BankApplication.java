...
//            , 예금(deposit), 출금(draw), 종료(exit) }
    ...
    public void addAccount(String accountNumber, String accountName, int money) {
        Account ac = new Account(accountNumber, accountName, money);
        accounts[getAccountSize()].setAccoutNumber(ac.getAccoutNumber());
        accounts[getAccountSize()].setAccountName(ac.getAccountName());
        accounts[getAccountSize()].setMoney(ac.getMoney());
    }
        ...
        for ( int i = 0; i < this.accounts.length; i++ ) {
            if( this.accounts[i] != null ) {
                System.out.printf("%s \t %s \t %d \n"
                        , this.accounts[i].getAccoutNumber()
                        , this.accounts[i].getAccoutNumber()
                        , this.accounts[i].getMoney()
                );
            }
        }
    }
    ...
    public void deposit(String accountNumber, int money) {
        for(int i = 0; i < getAccountSize(); i++) {
            if(accounts[i].getAccoutNumber().equals(accountNumber)) {
